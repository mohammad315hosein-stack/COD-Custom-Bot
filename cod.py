...
def migrate_db(conn):
    ...
    # All newly created customs use the new 5-minute lock.
    ...
    if state:
        ...
        if state["action"] == "admin_balance":
            ...
            with conn:
                ...
                security_log("balance_adjust", actor_id=user_id, target_user_id=target_id, details=f"تغییر {number:+d} سکه", conn=conn)
...
def process_callback(
    ...
    # -----------------------------------------------------
        ...
        success, result = submit_results(
            custom_id,
            state["selected"],
            allow_correction=(state.get("action") == "results_edit")
        )
